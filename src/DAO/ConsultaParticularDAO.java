...
public class ConsultaParticularDAO implements RepositorioConsultaParticular {
	private Connection connection;

	public ConsultaParticularDAO() {
		this.connection = new Conexao().getConnection();
	}

	public boolean adicionarConsultaParticular(ConsultaParticular consulta) {
		String sql = "insert into consulta(crm_medico,cpf_paciente,hora,dataa,consulta_particular) values (?,?,?,?,?)";
		try {
			...
			stmt.setString(5, consulta.getValor());
	...
	public ConsultaPorPlano buscarConsultaParticular(String consulta_particular) {
		ConsultaPorPlano plano = null;
		String sql = "select consulta_particular from consulta where consulta_particular=?";
		try {
			PreparedStatement stmt = connection.prepareStatement(sql);
			
			stmt.setString(1, consulta_particular);
	...
	public boolean atualizarConsultaPorPlano(ConsultaPorPlano plano, String cnpjPlano) {
	...
	public boolean buscarDataHora(String dataa, String hora){
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Data ou Hora incorreta!" + e.getMessage());
			return false;
		}
		return fim;
	}
